...
declare -a FairnessThreshList=(0.8 0.9 0.95)
# declare -a FairnessThreshList=(0.5 0.6 0.7)
# declare -a FairnessThreshList=(0.8)

# Alpha values list
# declare -a AlphaList=(2.0 1.8 1.6 1.4 1.2 1.05)
# declare -a AlphaList=(1.2)
declare -a AlphaList=(1.05)
        ...
        for seed in ${RandSeedList[@]}; do
            ...
            for data in ${DataList[@]}; do
                ...
                for sens in ${SensList[@]}; do
                    ...
                    outf="eval.adult.r.$seed.a.$alpha.f.$fair.s.$sensID.i.$dataID"
                    echo "START ------------------------------------------------------"
                    
                    # define the command with all the parameters as a string variable
                    # CMD="ls"
                    CMD="-r $seed -a $alpha -f $fair -s $sens -i $data --eval-file-output ./rq5-adult-tree/${outf}.pkl"
                    
                    # echo the command we will run: very useful for debugging
                    echo "Running python adult_patch.py with args: $CMD" 

                    # RUN the command
                    timeout 1800 python ./adult_patch.py $CMD &> ./rq5-adult-tree/${outf}.eval
