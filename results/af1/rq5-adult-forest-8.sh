            ...
            for size in ${ForestSize[@]}; do
                ...
                for data in ${DataList[@]}; do
                    ...
                    for sens in ${SensList[@]}; do
                        ...
                        # CMD="ls"
                        CMD="-r $seed -a $alpha -f $fair -s $sens -i $data -t $size --eval-file-output ./rq5-adult-forest/${outf}.pkl"
                        
                        # echo the command we will run: very useful for debugging
                        echo "Running python adult_patch.py with args: $CMD" 

                        # RUN the command
                        python3 ./adult_patch.py $CMD &> ./rq5-adult-forest/${outf}.eval
