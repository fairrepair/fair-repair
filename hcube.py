...
class HCube:
    """
    Represents a path hypercube, which is a set of constraints on features,
    and a value for the classification of points that fall into the 
    hypercube.

    Assumption is that if the feature has no constraints, then the hypercube accepts any values for this feature
    """
    def __init__(self,constraints=None,val=None,hid=None,pts=None,desc=None,data_list=None):
        """
        map: str (feature) -> list of [X,constraint] pairs

        This defines the hypercube as constraints on the feature/dimensions of the dataset. A feature may be constrained multiple times.

        if X is True then the constraint is an upper bound (i.e., <=)
        if X is False then the constraint is a lower bound (i.e., >)
        
        1. constraint is simply a double value/threshold
        2. hid is the hypercube-id (string) that uniquely identifies this
        hypercube in the tree where the hypercube originated.
        3. desc is the description of the path conditions of the HCube.
        """
        ...
        for point in self.pts:
            self.passing_rate += data_list[point]['frequency']
    ...
    def refine_one_hot(self, attr_to_refine,data_list):
        """
        ...
        for point in points_copy:
            if not c_lower.satisfies(data_list[point][attr_to_refine]):
                ...
                h_new.add_point(data_list[point])
                # Remove this point from this hypercube
                self.remove_point(data_list[point])
        return h_new

    def refine_cont_one_hot(self,attr_to_refine,thresh,data_list):
        """
        Invoked on an HCube instance.
        1. Changes the current HCube to include a lower bound on attr_to_refine
        2. Returns a new HCube that is created based off self, and includes an upper bound constraint on attr_to_refine
