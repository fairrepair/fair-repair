            ...
            for size in ${ForestSize[@]}; do
                ...
                for data in ${DataList[@]}; do
                    ...
                    for sens in ${SensList[@]}; do
                        ...
                        outf="eval.german.r.$seed.a.$alpha.f.$fair.s.$sensID.i.$dataID.t.$size"
                        echo "START ------------------------------------------------------"
                        
                        # define the command with all the parameters as a string variable
                        CMD="-r $seed -a $alpha -f $fair -s $sens -i $data -t $size --eval-file-output ./gf/${outf}.pkl"
                        
                        # echo the command we will run: very useful for debugging
                        echo "Running python german_patch.py with args: $CMD" 

                        # RUN the command
                        python3 ./german_patch.py $CMD &> ./gf/${outf}.eval
